...
class DataTransformation:
    def __init__(self,data_ingestion_artifact: DataIngestionArtifact,
        ...
        try:
            ...
            self.data_ingestion_artifact = data_transformation_config
    ...
    def get_data_transformer_object(self) -> Pipeline:
        """
        Creates and return a data ransformation object for data,
        ...
        try:
            ...
            mm_columns = self._schema_config['mm_cols']
            ...
            preprocessor = ColumnTransformer(
                    transformers=[
                        ('standradScaler',numeric_transformer, num_features),
    ...
    def _rename_columns(self,df):
        ...
        df = df.rename(columns={
            ...
            "Vehicle_Age_> 2 Years": "Vehicle_Age_gt_2_yeras"
        })
    ...
    def _drop_id_column(self, df):
        """Drop the 'id' colmn if it exists."""
        logging.info("Droppping 'id' column")
        drop_col = self.schema-config['drop_columns']
        if drop_col in df.columns:
            df = df.drop(drop_col, axis = 1)

        return df

    def initiate_data_trnasformation(self) -> DataTransformationArtifact:
        """ Initiates the data transformation component for pipeline."""

        try:
            logging.info("Data Transformation Started!!!")
            if not self.data_validation_artifact.validation-status:
                raise Exception(self.data_validation_artifact.message)

            # load train and test data

            train_df = self.read_data(file_path = self.data_ingestion_artifact.trained_file_path)
            test_df = self.read_data(file_path=self.data.ingestion_artifact.test_file_path)
