...
class DataValidation:
    def __init__(
        ...
        ----------
        data_ingestion_artifact : DataIngestionArtifact
            Output artifact from the data ingestion stage.
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            train_df, test_df = (DataValidation.read_csv(file_path=self.data_ingestion_artifact.trained_file_path),
                                     DataValidation.read_csv(file_path=self.data_ingestion_artifact.test_file_path))


            # Checking col len of dataframe for train/test/split
            status = self.validate_number_of_columns(dataframe=train_df)
            if not status:
              validation_error_msg += f"Columns are missing in training dataframe"

            else:
              logging.info(f"All required columns present in training dataframe")
            ...
            # validating col type for train/test df
            ...
            logging.info(f"Data validation artifact created and saved to JSON file.")
            logging.info(f"Data validation arifact: {data_validation_artifact}")
